//! 业务数据存储抽象层 — 图层要素 (业务数据) 的持久化。
//!
//! 与元数据存储 ([`crate::store::Store`]) 分离, 便于业务数据使用独立的
//! 数据库 / NFS / 对象存储等后端。后端通过 [`crate::config::BusinessConfig::kind`] 选择:
//! - `local` — 本地目录 (每图层一个 GeoJSON 文件, 支持 NFS/对象存储挂载)
//! - `metadata` — 复用元数据存储 (内置默认选项; 元数据为外部存储时的默认)
//! - `postgres` — 独立 PostgreSQL 业务存储
...
#[async_trait]
impl BusinessStore for SqliteStore {
    async fn save_features(&self, layer_name: &str, features: &[Feature]) -> Result<usize, StoreError> {
...
/// 构建业务数据存储。
///
/// 按 [`GeoServerConfig::effective_business`] 解析出的生效配置选择后端。
/// 返回 `None` 表示业务存储不可用 (如后端初始化失败)。
pub async fn build_business_store(config: &GeoServerConfig) -> Option<Arc<dyn BusinessStore>> {
    let bc = config.effective_business();
    match bc.kind.as_str() {
        // 复用元数据存储 (内置默认选项; 元数据为非 sqlite 外部存储时的默认)
        "metadata" => match config.metadata.kind.as_str() {
            "postgres" => match postgres::PostgresBusinessStore::new(&config.metadata.postgres).await {
                Ok(s) => Some(Arc::new(s)),
                Err(e) => {
                    eprintln!("Failed to initialize business store (reuse postgres metadata): {}", e);
                    None
                }
            },
            _ => {
                ...
                match SqliteStore::new(sqlite_path).await {
                    Ok(s) => Some(Arc::new(s)),
                    Err(e) => {
                        eprintln!("Failed to initialize business store (reuse sqlite metadata): {}", e);
                        None
                    }
                }
            }
        ...
        // 独立 PostgreSQL 业务存储
        "postgres" => match postgres::PostgresBusinessStore::new(&bc.postgres).await {
            Ok(s) => Some(Arc::new(s)),
            Err(e) => {
                eprintln!("Failed to initialize business store (postgres): {}", e);
                None
            }
        },
        // 本地目录 (默认)
        _ => {
            let dir = bc.dir.clone().unwrap_or_else(|| config.data_dir.join("business"));
            Some(Arc::new(local_dir::LocalDirBusinessStore::new(dir)))
        }
    }
}
