...
pub async fn list_sql_views(state: web::Data<AppState>) -> Result<HttpResponse, GeoServerError> {
    if let Some(store) = &state.store {
        match store.get_all_sql_views().await {
            Ok(views) => {
                let result: Vec<_> = views.iter().map(|v| serde_json::json!({
                    "name": v.name,
                    "sql": v.sql,
            ...
            Err(e) => {
                ...
                Err(GeoServerError::InternalError("Failed to list SQL views".to_string()))
            }
        }
    } else {
...
pub async fn get_sql_view(
    ...
    if let Some(store) = &state.store {
        match store.get_sql_view(name).await {
            ...
            Ok(None) => Err(GeoServerError::NotFound(format!("SQL view '{}' not found", name))),
            Err(e) => {
                eprintln!("Failed to get SQL view: {}", e);
                Err(GeoServerError::InternalError("Failed to get SQL view".to_string()))
            }
        }
    } else {
        Err(GeoServerError::NotFound(format!("SQL view '{}' not found", name)))
    }
}
...
pub async fn create_sql_view(
    ...
    if let Some(store) = &state.store {
        ...
        if let Ok(Some(_)) = store.get_sql_view(&body.name).await {
            return Err(GeoServerError::Conflict(format!("SQL view '{}' already exists", body.name)));
        }
        ...
        let view = SqlView {
            ...
            geometry_column: body.geometry_column.clone().unwrap_or_else(|| "geom".to_string()),
            geometry_type: body.geometry_type.clone().unwrap_or_else(|| "Geometry".to_string()),
        ...
        match store.create_sql_view(&view).await {
            Ok(_) => {
                // 同时创建一个虚拟图层
                info!("[SQL View] Creating virtual layer '{}' from SQL view", view.name);
                let layer = crate::models::Layer::new(
                    view.name.clone(),
                    view.description.clone().unwrap_or_else(|| view.name.clone()),
            ...
            Err(e) => {
                ...
                Err(GeoServerError::InternalError("Failed to create SQL view".to_string()))
            }
        }
    } else {
...
pub async fn update_sql_view(
    ...
    if let Some(store) = &state.store {
        match store.update_sql_view(
            name, body.sql.clone(), body.geometry_column.clone(),
            body.geometry_type.clone(), body.crs.clone(),
            body.parameters.clone(), body.description.clone(),
        ).await {
            Ok(_) => {
            ...
            Err(e) => {
                ...
                Err(GeoServerError::InternalError("Failed to update SQL view".to_string()))
            }
        }
    } else {
        Err(GeoServerError::InternalError("Database not available".to_string()))
    ...
    if let Some(store) = &state.store {
        match store.delete_sql_view(name).await {
            ...
            Err(e) => {
                ...
                Err(GeoServerError::InternalError("Failed to delete SQL view".to_string()))
            }
        }
    } else {
        Err(GeoServerError::InternalError("Database not available".to_string()))
...
pub async fn preview_sql_view(
    ...
    let data_source = {
        ...
        if let Some(ref store) = store_lock {
            store.get_data_source(&body.store).await
    ...
    let ds = data_source.ok_or_else(|| GeoServerError::NotFound(format!("Data source '{}' not found", body.store)))?;

    if ds.data_source_type != crate::models::DataSourceType::Postgis {
        return Err(GeoServerError::BadRequest("SQL View 仅支持 PostGIS 数据源".to_string()));
    }
    ...
    let rows = client.query(&limited_sql, &[]).await
    ...
    if rows.first().is_some() {
        let names = client
            ...
            .await
            .map(|stmt| {
                stmt.columns().iter().map(|c| serde_json::json!({
                    "name": c.name(),
                    "type": format!("{:?}", c.type_()),
                })).collect::<Vec<_>>()
            })
