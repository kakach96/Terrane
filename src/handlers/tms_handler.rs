...
//!
//! Routes (under `{api_context}`, e.g. `/geoserver`):
    ...
    match request.as_str() {
        ...
        "GETTILE" => {
            let path = tms::parse_kvp_tile(params).ok_or_else(|| {
                GeoServerError::BadRequest("Missing TMS GetTile parameters".to_string())
            })?;
...
pub async fn handle_tms_path(
    ...
    let (layer_name, gridset, format) = tms::parse_tilemap_id(tail)
        .ok_or_else(|| GeoServerError::BadRequest("Invalid TMS path".to_string()))?;
    let layer = layers
        .iter()
    ...
    let doc = tms::build_tile_map(&base, layer, &gridset, &format)
        .ok_or_else(|| GeoServerError::BadRequest("Unknown TMS gridset or format".to_string()))?;
...
async fn render_tms_tile(
    ...
    let row_slippy = tile_grid::tms_row_to_slippy(&path.gridset, path.z, path.y_tms)
        .ok_or_else(|| GeoServerError::BadRequest("TMS tile row out of range".to_string()))?;
