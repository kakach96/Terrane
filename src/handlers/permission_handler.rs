...
pub async fn list_permissions(
    ...
    if let Some(store) = &state.store {
        match store.get_permissions().await {
            Ok(perms) => {
                let result: Vec<_> = perms.iter().map(|p| serde_json::json!({
                    "id": p.id,
                    "username": p.username,
                    "role": p.role,
            ...
            Err(e) => Err(GeoServerError::InternalError(format!("查询权限失败: {}", e))),
        }
    } else {
...
pub async fn create_permission(
    ...
    let perm = Permission {
        ...
        resource_name: body.resource_name.clone().unwrap_or_else(|| "*".to_string()),
    ...
    if let Some(store) = &state.store {
        match store.create_permission(&perm).await {
            Ok(id) => {
                info!("[Permission] 创建权限: id={}, user={}, resource={}/{}",
            ...
            Err(e) => Err(GeoServerError::InternalError(format!("创建权限失败: {}", e))),
        }
    } else {
        Err(GeoServerError::InternalError("数据库不可用".to_string()))
    }
...
pub async fn delete_permission(
    ...
    require_auth(&req)?;
    let id: i64 = req.match_info().get("id").and_then(|v| v.parse().ok())
        .ok_or_else(|| GeoServerError::BadRequest("无效的权限 ID".to_string()))?;

    if let Some(store) = &state.store {
        store.delete_permission(id).await
...
pub async fn check_permission_handler(
    ...
    let required_mode = req.query_string().split('&')
        .find_map(|pair| {
            let mut parts = pair.splitn(2, '=');
            if parts.next()? == "mode" { parts.next() } else { None }
        })
        .unwrap_or("read");

    if let Some(store) = &state.store {
        match store.check_permission(&claims.sub, &claims.role, resource_type, resource_name, required_mode).await {
            Ok(allowed) => {
                Ok(HttpResponse::Ok().json(ApiResponse::success(serde_json::json!({
                    "allowed": allowed,
                    "username": claims.sub,
            ...
            Err(e) => Err(GeoServerError::InternalError(format!("权限检查失败: {}", e))),
        }
    } else {
        Err(GeoServerError::InternalError("数据库不可用".to_string()))
    }
