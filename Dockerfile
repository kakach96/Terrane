...
# =============================================================================
# Rust GeoServer - multi-stage container image
#
# Stages:
#   1. frontend : build the Angular frontend with Node 20 -> /app/dist/rust-geoserver-ui
#   2. builder  : compile the Rust backend -> /build/target/release/rust-geoserver
...
# Artifacts: /app/dist/rust-geoserver-ui (angular.json -> outputPath)
...
COPY --from=builder   /build/target/release/rust-geoserver /app/rust-geoserver
COPY --from=frontend  /app/dist/rust-geoserver-ui          /app/static
...
ENV GEOSERVER__SERVER__HOST=0.0.0.0 \
    ...
    GEOSERVER__SECURITY__JWT_SECRET=rust-geoserver-dev-secret \
    RUST_LOG=info
