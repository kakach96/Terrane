...
Endpoint `/wms` (`service/src/services/wms.rs`, `service/src/handlers/wms_handler.rs`).
...
Endpoint `/wfs` (`service/src/services/wfs.rs`, `service/src/handlers/wfs_handler.rs`). GET + POST.
...
Endpoint `/wcs` (`service/src/services/wcs.rs`, `service/src/handlers/wcs_handler.rs`).
...
### 5.1 WMTS 1.0.0 ✅
Endpoint `/wmts` (`service/src/services/wmts.rs`, `service/src/handlers/wmts_handler.rs`).
...
- `/tiles/{layer}/{z}/{x}/{y}.pbf`, `/mvt/{layer}/{z}/{x}/{y}` — pure-Rust protobuf
  encoder (`service/src/utils/mvt.rs`, `service/src/handlers/mvt_handler.rs`)

### 5.3 Tile cache (GWC-like) ✅

- `/tiles/{layer}/{z}/{x}/{y}` with a local disk cache (`<data_dir>/gwc`,
  `TileCacheBackend` trait, `service/src/store/cache/`) + layer-level Redis data-source
  backends (`Layer.cache_store` → `DataSourceType::Redis`)
- **Seeding / truncate** (`service/src/utils/tile_seed.rs` + `/tiles/seed` REST): background
...
- **Custom gridsets**: runtime registration (per-zoom resolutions), honoured by
  the shared grid math (`service/src/utils/tile_grid.rs`)
...
paths) and reuse the shared tile engine (`service/src/handlers/tile_common.rs`) and grid
math (`service/src/utils/tile_grid.rs`):
...
Endpoint `/wps` (`service/src/services/wps.rs`, `service/src/handlers/wps_handler.rs`). First WPS
...
convention: every file directly under `service/tests/` is its own binary), sharing a
`service/tests/common/mod.rs` helper (`create_test_config` + `build_test_app!` macro):
...
| `service/tests/wmts_test.rs`| 4     | WMTS (GetCapabilities / GetTile / GetFeatureInfo)         |
| `service/tests/tms_test.rs` | 7     | TMS (GetCapabilities RESTful + KVP, TileMap document, GetTile geodetic/mercator PNG + JPEG, KVP GetTile) |
| `service/tests/wps_test.rs` | 6     | WPS (GetCapabilities, DescribeProcess, Execute KVP raw centroid/buffer/bounds + XML POST) |
| `service/tests/csw_test.rs` | 10    | CSW (GetCapabilities, DescribeRecord, GetRecords KVP + XML POST summary/brief/full + CQL constraint + paging/hits, GetRecordById, GetDomain) |
| `service/tests/ogc_api_test.rs` | 8 | OGC API Features (landing, conformance, collections, collection, items with limit/offset/bbox, item by id, 404) |
| `service/tests/ogc_tiles_test.rs` | 7 | OGC API Tiles (landing, conformance, tileMatrixSets + definition, collections/tilesets, tile PNG/JPEG, 404) |
| `service/tests/ogc_maps_test.rs` | 8 | OGC API Maps (landing, conformance, collections, collection, styles, map PNG/JPEG + size, 400/404) |
| `service/tests/ogc_processes_test.rs` | 9 | OGC API Processes (landing, conformance, process list, description, execute centroid/buffer, jobs list, results, cancel 409, 400/404) |
| `service/tests/ogc_coverages_test.rs` | 9 | OGC API Coverages (landing, conformance, collections empty/with real GeoTIFF, collection detail with real 8x8 metadata + bounds, coverage GeoTIFF/PNG/JPEG, bbox crop 2x2 + resize 8x8, disjoint bbox 400, 404) |
| `service/tests/ogc_styles_test.rs` | 7 | OGC API Styles (landing, conformance, style list + SLD content + 404, metadata, create 401/201, get/put(CSS content-type)/delete lifecycle, collections + layer styles) |
| `service/tests/resilience_test.rs` | 6 | Resilience middleware (rate limit 429, per-client X-Forwarded-For buckets, request timeout 504, fast requests pass) + **TraceId echo regression** (X-Trace-Id passthrough + generated) + **full middleware stack** (TraceId + RateLimit + RequestTimeout + Compress serves without panic) |
...
| WPS                | GetCapabilities (ServiceIdentification / OperationsMetadata / ProcessOfferings / Languages), DescribeProcess (DataInputs / ProcessOutputs + xsd:double), Execute (KVP `response=raw` → GeoJSON + document XML, POST XML with `Reference xlink:href="layer:…"`), built-in `vec:Centroid` / `vec:Buffer` / `gs:Bounds` — integration + 6 unit tests (`service/src/services/wps.rs`: KVP DataInputs, operation parse, Execute XML, capabilities structure, features_to_geojson, run_process) |
| OGC API Maps       | Landing / conformance / collections / collection / styles, `map` operation (`bbox` + `width`/`height`, PNG/JPEG via `?f=`, `transparent` / `bgcolor` / `datetime` / `cql_filter` pass-through) reusing the shared WMS GetMap pipeline — integration + 8 unit tests (`service/src/services/ogc_maps.rs`: landing, conformance, collections, collection links, styles, bbox parse, map href formats) |
| OGC API Processes  | Landing / conformance / process list / process description; synchronous job surface (`POST /jobs` → 201 status document, `GET /jobs`, `GET /jobs/{id}`, `GET /jobs/{id}/results`, `DELETE /jobs/{id}`) over the WPS engine, inputs as `layer:` reference / inline GeoJSON / OGC API href / literals — integration + 9 unit tests (`service/src/services/ogc_processes.rs`: landing, conformance, process list/description, job status/results, job request parse) |
| OGC API Coverages | Landing / conformance / collections (empty + with a real 8x8 georeferenced GeoTIFF data source) / collection detail (real bounds + grid scale + band fields) / `coverage` operation (GeoTIFF default, PNG + JPEG via `?f=`, bbox crop 2×2 + width/height resample 8×8, disjoint bbox 400, unknown collection 404) — integration + 7 unit tests (`service/src/services/ogc_coverages.rs`: landing, conformance, collections, collection links/dimensions, bbox parse, coverage hrefs) |
| OGC API Styles     | Landing / conformance / style list (built-in `default` SLD) + style content with native media type + 404 / metadata / create (401 without token, 201 with JWT) / get–put (CSS content-type after replace)–delete lifecycle / collections + layer styles — integration + 8 unit tests (`service/src/services/ogc_styles.rs`: mime mapping, landing, conformance, list, metadata, collections, layer-style resolution, hrefs) |
| Resilience (middleware) | Rate limit (429 over limit, per-client `X-Forwarded-For` buckets) + request timeout (504 over a real HTTP server, fast requests pass) + **TraceId echo** (X-Trace-Id passthrough / generated) + **full production middleware stack** (TraceId + RateLimit + RequestTimeout + Compress serves without panic — regression for the `HeaderName::from_static` uppercase panic) — integration + 5 unit tests (`service/src/middleware.rs`: allow/deny, disabled limiter, per-client independence, window slide, count after prune) |
...
| Tile grid (util)   | 5 unit tests (`service/src/utils/tile_grid.rs`: geodetic matrix/bounds, mercator matrix/bounds, TMS y-flip, zoom-for-resolution, bbox→tile) |
...
| Tile cache (util)  | 15 unit tests (`service/src/utils/tile_cache.rs` + `service/src/store/cache/tile.rs`: gridset, bounds, hit-rate, **metastore write + fast-path stats**, **layer quota LRU eviction**, put/get/clear) + 2 `service/src/utils/tile_seed` (serde, status) + 7 `service/src/utils/tile_grid` (geodetic/mercator + **custom gridset matrix/bounds/zoom**) |
| CQL / ECQL         | 8 unit tests (`service/src/utils/cql_filter.rs`) — incl. bug8 regression: `A AND B` is a real AND, not OR |
| Projection         | 7 unit tests (`service/src/utils/projection.rs`)                                     |
| Geometry           | 3 unit tests (`service/src/utils/geometry.rs`)                                       |
| Shapefile          | 3 unit tests (`service/src/utils/shapefile.rs`: PRJ + coordinates)                   |
| Shapefile export   | 4 unit tests (`service/src/utils/shapefile_export.rs`: .shp/.shx structure, .dbf fields/values, .prj, ZIP package, **round-trip through the reader**, polyline/polygon shape types) |
| GeoTIFF            | 3 unit tests (`service/src/utils/geotiff.rs`: format, crop-no-geo, georeferencing-tags → bounds + crop) |
| Config             | 3 unit tests (`service/src/config.rs`, incl. legacy `[business]`/`[gwc]` alias)      |
| Auth               | 8 unit tests (`service/src/auth.rs`: salt, hash, verify, JWT)                        |
| Upload             | 1 unit test (`service/src/handlers/upload_handler.rs`: filename sanitize)                     |
...
| Store (vector/raster) | 4 `LocalVectorStore` (save/load/delete/list/sanitize) + 4 `LocalRasterStore` (put/get/delete/list/.tif) + 8 `service/src/store/sqlite_store` (workspace / namespace / layer+features / user+permission / session / styles CRUD / layer-groups CRUD / audit logs) + 2 live `PostgresStore` (metadata CRUD) / `PostgresVectorStore` (feature round-trip) — `#[ignore]` |
| Data-source adapters | 4 `service/src/utils/arcgrid` (read/meta/errors) + 5 `service/src/utils/worldimage` (ext/meta/crop) + 5 `service/src/utils/cascaded` (config extract + **vendor-param URL encoding** + live `fetch_cascaded_map` via WMS proxy: CQL_FILTER valid/invalid + TIME pass-through) + 8 `service/src/utils/geopackage` (layers / validation / features read round-trip / limit + **write→read round-trip** for Point & LineString + **typed attributes: INTEGER/REAL/BOOLEAN/TEXT inference + round-trip** + **stable feature ids `table.rowid`**) + 2 `service/src/models/data_source` (serde type round-trip incl. `cascaded_wms`, postgis connection constructor) + 4 `service/src/utils/wkb` (Point/LineString/Polygon round-trip + **Multi*/GeometryCollection round-trip** + byte lengths + big-endian decode, all 7 WKB types now parse) + 7 `service/src/utils/wfs_lock` (acquire / conflict ALL / SOME skip / expiry / never-expire / renew+release / per-layer) + 3 `service/src/utils/gml` (escape / point / feature GML 3.2) |
| Security           | 12 `service/src/utils/geofence` (no-rules allow / anonymous deny / allow rule / workspace + store scopes / qualified layer name / specific-beats-wildcard / deny-wins-tie / priority tiebreak / mode hierarchy / service-global) + 7 `service/src/utils/ldap` (filter substitution / DN escaping / DN building / role mapping / parse-role / disabled config) + 6 `service/src/utils/secrets` (no-placeholder / env expand / missing env / unterminated / connection resolve / redact) + 3 `service/src/config` (ldap defaults / ldap parse / geofence parse) |
...
- **WCS `DescribeCoverage` always returned 500** — quick-xml cannot serialize a bare
  ...
  `service/src/handlers/wcs_handler.rs` by serializing each `CoverageDescription` and joining.
- **WFS Transaction Delete never deleted anything** — `parse_transaction_xml` always
  ...
  `service/src/services/wfs.rs` by collecting `fid` attributes from `ogc:FeatureId` elements
...
- **ArcGrid header parsing started data at the wrong line** — `read_arcgrid` only
  ...
  count never matched `nrows * ncols`. Fixed in `service/src/utils/arcgrid.rs` by advancing
...
- **WCS `DescribeCoverage` only enriched GeoTIFF metadata** — ArcGrid / WorldImage
  ...
  their real bounds / size. Fixed in `service/src/handlers/wcs_handler.rs` by matching on
...
- **`DataSourceType::CascadedWms` serde name was `cascadedwms`** — `#[serde(rename_all
  ...
  WMS data source over REST failed to deserialize. Fixed in `service/src/models/data_source.rs`
...
- **`PostgresVectorStore::delete_features` always returned 0** — it issued
  ...
  deleted-row count. Fixed in `service/src/store/vector/postgres.rs` to `DELETE` rows and
...
- **GeoTIFF georeferencing tags were never read (bug7)** — `try_read_geotiff_tags_native`
  ...
  the full image. Fixed in `service/src/utils/geotiff.rs` by using the named tag variants;
  guarded by a new unit test (`service/src/utils/geotiff.rs`) that writes a georeferenced TIFF
...
- **CQL/ECQL `A AND B` parsed as `A OR B` (bug8)** — `split_top_level` built its
  ...
  asserted `true` and never caught it. Fixed in `service/src/utils/cql_filter.rs` by passing
...
`geopackage_table_columns` helper in `service/src/utils/geopackage.rs`) instead of the
hardcoded id/name/geometry stub. The SQLite→XSD mapping (`sqlite_type_to_xsd` in
`service/src/services/wfs.rs`) follows the reference GeoServer: INTEGER→`xsd:long`, REAL→
...
surface now shares one grid helper (`service/src/utils/tile_grid.rs`: EPSG:4326
...
(`service/src/handlers/tile_common.rs`, PNG + JPEG, cache on PNG only), which the WMTS
...
new hand-written in-memory exporter `service/src/utils/shapefile_export.rs` produces
...
`service/src/services/wps.rs` + `service/src/handlers/wps_handler.rs` served at `/wps`
...
`service/src/services/csw.rs` + `service/src/handlers/csw_handler.rs` served at `/csw`
...
surface** — `service/src/services/ogc_features.rs` + `service/src/handlers/ogc_api_handler.rs`
...
`service/src/services/ogc_tiles.rs` + `service/src/handlers/ogc_tiles_handler.rs` served at
...
(OGC 18-062) first surfaces** — `service/src/services/ogc_maps.rs` +
`service/src/handlers/ogc_maps_handler.rs` served at `/ogc/maps` and
`service/src/services/ogc_processes.rs` + `service/src/handlers/ogc_processes_handler.rs`
...
`render_ogc_map` helper in `service/src/handlers/wms_handler.rs`), so OGC API - Maps serves the
...
(OGC 21-009) first surfaces + resilience hardening** — `service/src/services/
ogc_coverages.rs` + `service/src/handlers/ogc_coverages_handler.rs` served at
`/ogc/coverages` and `service/src/services/ogc_styles.rs` + `service/src/handlers/
...
`service/src/middleware.rs` adds a sliding-window per-client rate limiter (HTTP 429,
...
`service/src/utils/cascaded.rs` adds exponential-backoff retry for cascaded WMS upstreams
...
(`service/src/store/cache/redis.rs` + `RedisTileCacheBackend`, resolved per layer in
