...
- **Frontend build**: `optimization.fonts = false` (offline env); **zero external runtime dependencies** — no Google Fonts / CDN requests (system font stack in `styles.scss`; OpenLayers for the WMS preview iframe is bundled at `frontend/src/assets/vendor/ol/` and served from `/assets/vendor/ol/`)
