...
frontend/         — Angular 22 + Material
  src/app/
    models/       — TypeScript interfaces (terrane.models.ts)
    services/     — terrane.service.ts (API client)
...
- **API base path**: `/terrane` (configurable in `service/terrane.toml: api_context`)
...
- **Built-in sample data** (`service/samples/` + `service/src/utils/samples.rs`): on first startup (empty catalog) the backend copies the curated GeoJSON files (`major_cities` points / `sample_routes` lines / `world_countries` simplified polygons) into `<data_dir>/samples/` and auto-registers a `demo` workspace with one GeoJSON data source + layer per dataset, so the product demos out-of-the-box. Opt-in via `[samples] enabled` (default true) / `TERRANE__SAMPLES__ENABLED`; `source_dir` defaults to `./samples`. Seeding only runs when the catalog has no layers (idempotent — existing installs are never modified). The Docker image ships `samples/` at `/app/samples`.
...
- **Config**: `service/terrane.toml` optional; defaults work without it. Environment variables: `RUST_LOG`, `TERRANE__SERVER__HOST` etc. (double-underscore separator). `load_from_file()` already mounts the `TERRANE__` env source, so env overrides also work with `--config`.
...
- **Containerized**: multi-stage `Dockerfile` (node → rust → debian-slim, non-root) + `.dockerignore` + `build/docker-compose.yml`; GitHub Actions CI (`.github/workflows/ci.yml`: fmt + clippy + test + frontend build + GHCR image push + Trivy scan) + Dependabot — created, not yet exercised against a real repository
  ```bash
  ...
  ```
- **JWT secret has a hardcoded default** in `service/src/auth.rs` (`terrane-jwt-secret-2026`); inject via env `TERRANE__SECURITY__JWT_SECRET` in multi-replica prod (docker-compose passes `TERRANE_JWT_SECRET`)
- **Runtime host**: default `127.0.0.1`; Docker image sets `TERRANE__SERVER__HOST=0.0.0.0`
- **State**: metadata in SQLite (`service/terrane.sqlite`) + vector data (default local dir `<data_dir>/business`, or PostgreSQL) + raster files (default `<data_dir>/rasters`) + in-memory caches (`Arc<RwLock<...>>` in `service/src/state.rs`) + session cache (in-memory) + local disk tile cache (`service/data/gwc`) + uploads (`service/data`) → multi-replica needs shared storage or PostgreSQL / object-storage backends
