...
- **Built-in sample data** (`service/samples/` + `service/src/utils/samples.rs`): on first startup (empty catalog) the backend copies the curated GeoJSON files (`major_cities` points / `sample_routes` lines / `world_countries` simplified polygons) into `<data_dir>/samples/` and auto-registers a `demo` workspace with one GeoJSON data source + layer per dataset, so the product demos out-of-the-box. Opt-in via `[samples] enabled` (default true) / `GEOSERVER__SAMPLES__ENABLED`; `source_dir` defaults to `./samples`. Seeding only runs when the catalog has no layers (idempotent — existing installs are never modified). The Docker image ships `samples/` at `/app/samples`.
