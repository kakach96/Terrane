...
async fn test_wmts_get_capabilities() {
    ...
    assert!(resp.status().is_success(), "WMTS GetCapabilities 应返回 200, 实际: {}", resp.status());
...
async fn test_wmts_get_tile_kvp() {
    ...
    let req = test::TestRequest::get()
        ...
        .to_request();
    let resp = test::call_service(&app, req).await;
    assert!(resp.status().is_success(), "WMTS GetTile 应返回 200, 实际: {}", resp.status());
...
async fn test_wmts_get_tile_restful() {
    ...
    let req = test::TestRequest::get()
        ...
        .to_request();
    let resp = test::call_service(&app, req).await;
    assert!(resp.status().is_success(), "WMTS RESTful 瓦片应返回 200, 实际: {}", resp.status());
...
async fn test_wmts_get_feature_info() {
    ...
    let req = test::TestRequest::get()
        ...
        .to_request();
    let resp = test::call_service(&app, req).await;
    assert!(resp.status().is_success(), "WMTS GetFeatureInfo 应返回 200, 实际: {}", resp.status());
